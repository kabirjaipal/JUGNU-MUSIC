...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.isCommand()) {
    ...
    if (cmd) {
      ...
      let botChannel = interaction.guild.members.cache.get(client.user.id).voice.channel;
      let checkDJ = await check_dj(client, interaction.member, queue?.songs[0]);
      if (
        !interaction.member.permissions.has(
          cmd.userPermissions.reduce((prev, curr) => prev | Permissions[curr], 0)
        )
      ) {
        return client.embed(
          interaction,
        ...
        !interaction.guild.members.cache.get(client.user.id).permissions.has(
          cmd.botPermissions.reduce((prev, curr) => prev | Permissions[curr], 0)
        )
      ) {
        return client.embed(
          interaction,
