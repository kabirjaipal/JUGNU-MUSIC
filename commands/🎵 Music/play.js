...
module.exports = {
  ...
  run: async (client, message, args, song) => {
    ...
    functions.embedbuilder(client, 5000, message, config.colors.yes, "🎶  I AM Searching Your Song , Hehe!!", "```" + args.join(" ") + "```")
    ...
    //do things for spotify playlist
      ...
      let tracklength = items.length;
      if (tracklength > 25) {
        message.reply("the current maximum of tracks for spotify playlists are 25 tracks, if you wanna use bigger playlists, then dm `My Owner`\n\nI will use the first 25 songs!");
        tracklength = 25;
      }
      functions.embedbuilder(client, 5000, message, config.colors.yes, "🎶  Fetching the songs!", "This will take me around about: " + tracklength / 2 + " seconds");
      for (let i = 0; i < 25; i++) {
